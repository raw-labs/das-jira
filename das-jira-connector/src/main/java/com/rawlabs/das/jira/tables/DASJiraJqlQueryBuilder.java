...
import com.rawlabs.protocol.das.ListQual;
...
public class DASJiraJqlQueryBuilder {
  ...
  private Optional<String> maybeInCheck(ListQual qual) {
    List<Value> rawValues = qual.getValuesList();
    ...
    Operator rawOperator = qual.getOperator();
    boolean isAny = qual.getIsAny();
    if (rawOperator.hasEquals() && isAny) {
      // col EQUAL to ANY value IN (1,2,3) ==> IN
      StringJoiner joiner = new StringJoiner(", ", "IN (", ")");
      values.forEach(joiner::add);
      return Optional.of(joiner.toString());
    } else {
      if (rawOperator.hasNotEquals() && !isAny) {
  ...
  public List<Optional<String>> mkJql(List<Qual> quals) {
    ...
    for (Qual qual : quals) {
      String rawColumn = qual.getFieldName();
